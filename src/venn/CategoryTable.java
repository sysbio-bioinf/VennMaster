...
public class CategoryTable implements ListSelectionListener, ChangeListener {
	...
	private ExcelAdapter excelAdapter; // <- don't touch this!! This is a
										// decorator! It will get a compiler
										// warning!
	private boolean isTableChanging;
	private JSplitPane catSplit;
	private TableColumn pValueCol, nTotalCol, nChangeCol;
	...
	public CategoryTable(VennPanel venn) {
		...
		categoryTable.setDefaultRenderer(Object.class,
				new CategoryTableRenderer(venn));
		...
		categoryTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int row = table.rowAtPoint(e.getPoint()), col = table
						.columnAtPoint(e.getPoint());
				// System.out.println(row + ":"+col);
				if (row >= 0 && col >= 0) {
					col = table.convertColumnIndexToModel(col);
					if (col == 2) {
						CategoryTableData model = (CategoryTableData) table
								.getModel();
						AbstractGOCategoryProperties props = model
								.getGoProperties(row);
						if (props != null) {
		...
		selectionInfo.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() < 2)
					return;
		...
		catSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT, new JScrollPane(
				categoryTable), new JScrollPane(selectionInfo));
	...
	private void setPValueHeader(IVennDataModel model) {
		...
		for (int i = 0; i < numGroups; i++) {
			AbstractGOCategoryProperties prop = (AbstractGOCategoryProperties) model
					.getGroupProperties(i);
			if (label == null) {
	...
	private void setHeaders(AllParameters params) {
		if (params.logNumElements) {
			nTotalCol.setHeaderValue("nTotal log"
					+ Constants.WHICH_NELEMENTS_LOG);
			nChangeCol.setHeaderValue("nChanged log"
					+ Constants.WHICH_NELEMENTS_LOG);
		} else {
			nTotalCol.setHeaderValue("nTotal");
			nChangeCol.setHeaderValue("nChanged");
	...
	private void updateCategoryTableSelection() {
		...
		try {
			...
			categoryTable.setColumnSelectionInterval(0,
					categoryTable.getColumnCount() - 1);
	...
	public void stateChanged(ChangeEvent event) {
		...
		updateCategoryTableSelection(); // set table selection from view
										// selection
		updateSelectionInfo();
		return;
	}

	public void unschoenesStateChange() {
		
		
		invalidate();

//		if (isUpdating || isTableChanging) {
//			isTableChanging = false;
//			return;
//		}
	...
	private void updateSelectionInfo() {
		...
		try {
			...
			DefaultListModel listModel = (DefaultListModel) selectionInfo
					.getModel();
	...
	private String createGoURL(long termID) {
		...
		return "http://godatabase.org/cgi-bin/go.cgi?action=replace_tree&query=GO:"
				+ format.format(termID);
	}
